...
/*
connection.query('INSERT INTO logs_tokens (token) VALUES ("dato")',function(error,results){
    if(error) throw error;
    console.log("added",results)
})*/
/*
connection.query('SELECT * FROM logs_tokens', function (error, rows) {
 if(error){
    throw error
 }else{
...
//select top 1 *  from tbl order by id desc; // ASC
/*
var token_db = "";
...
function routes(app) {
    ...
    /*     console.log("running a task every 3 second");
         ...
         if (token_value == "empty") {
             ...
             //   console.log("token base ", token_base[0].token)
             //   console.log( await tokenVald(token_base[0].token));
             // console.log("funcuin", await generateToken());
             //await pesistDate()
    ...
    async function validoToken() {
        ...
        //const hora = moment.duration(fecha_Actual, fechaDeToken).get('minute');
    ...
    async function getInfo() {
        ...
        // console.log("resultado de get infor", results[0])
        return results[0]
    }
    ...
    async function generateToken() {
        try {
            ...
            /**`UPDATE logs_tokens
            SET token="${token_value}" WHERE id='17'` */
            /* connection.query(`INSERT INTO logs_tokens (token) VALUES ("${token_value}")`, function (error, results) {
                 if (error) throw error;
                 console.log("added", results)
             })*/
    ...
    router.get('/get-docs-trimester', async (req, res) => {
        try {
            ...
            const array = await getDocsSalesTrimestres(tokenValido);
